...
my $rrsetsElement = $doc->{ResourceRecordSets};

if(!$rrsetsElement || scalar @$rrsetsElement != 1) {
    die "Missing ResourceRecordSets elemen.t";
}

my $rrsetElements = $doc->{ResourceRecordSets}->[0]->{ResourceRecordSet};
